...
await runBrowserTest({
  ...
  async run({ page }) {
    ...
    const seeded = await page.evaluate(async (day) => {
      ...
      // CRITICAL: stamp the boot-pass version markers so the housekeeping
      // passes skip this DB.
      //
      // `recomputeClassificationsAndAccuracies` re-derives classifications AND
      // motifs from each move's stored FENs and evals — it owns that data, so
      // that a change to the motif rules re-stamps an existing library. Our
      // synthetic moves carry placeholder FENs and evals, so the pass happily
      // reclassifies them and wipes the `fork` / `pin` motifs this test is
      // asserting on. Observed directly: the Recommended summary rendered
      // correctly, then vanished ~1 s later when the pass completed.
      //
      // Version-stamping is the mechanism the app already provides for "this
      // DB is up to date, skip the pass", so we use it rather than fighting
      // the pass or hand-crafting analyzer-realistic positions.
    ...
    expect(Boolean(forkChip), `fork chip present (chips=${rec.chips.join(' | ')})`).toBe(true);
