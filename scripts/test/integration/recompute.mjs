...
await runBrowserTest({
  ...
  async run({ page }) {
    ...
    // Create a game + analysis with a stale accuracy to simulate old-formula output.
    ...
    // Give the boot-time recompute pass time to fire.
    await sleep(2000);
    ...
    expect(changed, 'accuracy changed after recompute').toBeTruthy();
  },
});
