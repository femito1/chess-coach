...
| `setup-r2-nnue.mjs` | `nnue:setup` | Stands up the whole R2 path through `wrangler`: create bucket → enable the r2.dev public URL → **read** that URL (the hash is account-specific, never construct it) → set the CORS rule → upload + verify → write `VITE_NNUE_NET_URL` to `.env.production`. Idempotent; `--dry-run` prints every command. Cannot enable R2 on the account (one-time dashboard opt-in, API error 10042) and cannot set a Pages build variable (`wrangler pages` has no such command) — both are reported with the fix. |
